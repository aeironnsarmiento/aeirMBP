...
const backfillRead = vi.hoisted(() => ({ failure: null as Error | null }));
vi.mock("@/widgets/music/server/backfill", async (importOriginal) => {
  const original = await importOriginal<
    typeof import("@/widgets/music/server/backfill")
  >();
  return {
    ...original,
    readBackfillProgress: (...args: Parameters<typeof original.readBackfillProgress>) =>
      backfillRead.failure
        ? Promise.reject(backfillRead.failure)
        : original.readBackfillProgress(...args),
  };
});
...
describe("unexpected settings failures", () => {
  it("logs read failures without returning datastore details", async () => {
    const { handleSettingsRead } = await import("../handlers");
    await asOwner();
    backfillRead.failure = new Error("database connection details");
    const log = vi.spyOn(console, "error").mockImplementation(() => {});

    const response = await handleSettingsRead();
  ...
  it("logs malformed input without returning parser details", async () => {
    const { handleSettingsUpdate } = await import("../handlers");
    await asOwner();
    const log = vi.spyOn(console, "error").mockImplementation(() => {});
    ...
    const response = await handleSettingsUpdate(request);

    expect(response.status).toBe(500);
    expect(await response.json()).toEqual({ error: "settings-update-failed" });
    expect(log).toHaveBeenCalledWith(

...
import shellStyles from "@/components/shell/Shell/Shell.module.css";
import sidebarStyles from "@/components/shell/Sidebar/Sidebar.module.css";
import widgetGridStyles from "@/components/shell/WidgetGrid/WidgetGrid.module.css";
...
describe("reflow below the desktop breakpoint (R6, R7)", () => {
  it("declares a single-column grid and a horizontal nav bar under 900px", () => {
    ...
    expect((sidebarStyles as unknown as Record<string, string>).sidebar).toBeTruthy();
    expect((widgetGridStyles as unknown as Record<string, string>).grid).toBeTruthy();
  });
  ...
  it("presents an expanded widget as a full-screen sheet below the breakpoint", () => {
    ...
    const source = readSibling("WidgetGrid/WidgetGrid.module.css");
  ...
  it("renders one tree at every width — no component reads the breakpoint", () => {
    const source = readSibling("WidgetGrid/WidgetGrid.tsx");
